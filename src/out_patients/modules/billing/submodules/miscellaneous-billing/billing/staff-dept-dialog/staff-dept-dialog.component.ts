...
export class StaffDeptDialogComponent implements OnInit {
  ...
  constructor(private formService: QuestionControlService, private http: HttpService, private dialogRef: MatDialogRef<StaffDeptDialogComponent>,) { }
  ...
  ngAfterViewInit(): void {
    ...
    this.question[1].elementRef.addEventListener("keypress", (event: any) => {
      if (event.key === "Enter") {
        ...
        this.http.get(ApiConstants.getstaffdependentdetails(this.staffForm.value.org, txt, txt))
          .pipe(takeUntil(this._destroying$))
          .subscribe((res: any) => {
            if (res) {

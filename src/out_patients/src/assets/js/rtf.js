if (typeof RTFJS === "undefined") {
  (typeof window !== "undefined" ? window : this).RTFJS = {
    ...
    _isdigit: function (str) {
      ...
      for (var i = 0; i < len; i++) {
        var ch = str.charCodeAt(i);
        if (ch < this._0 || ch > this._9) return false;
      }
      return len > 0;
    },
    _parseHex: function (str) {
      ...
      if (len > 0) return parseInt(str, 16);
      return NaN;
    },
    _blobToBinary: function (blob) {
      ...
      var ret = "";
      var len = view.length;
      for (var i = 0; i < len; i++) ret += String.fromCharCode(view[i]);
      return ret;
    },
...
RTFJS.RenderTextElement = function (doc, text, chp) {
  ...
  RTFJS.RenderElement.call(this, doc, "text", ele);
  this._chp = chp;
};

RTFJS.RenderTextElement.prototype = Object.create(
  RTFJS.RenderElement.prototype
);
...
RTFJS.RenderTextElement.prototype.applyProps = function () {
  ...
  if (chp.hasOwnProperty("fontfamily") && this._doc._fonts[chp.fontfamily]) {
    ...
    if (fontFamily !== "Symbol") el.style.fontFamily = fontFamily;
  }

  var deco = [];
  if (chp.underline != RTFJS.UNDERLINE.NONE) deco.push("underline");
  if (chp.strikethrough || chp.dblstrikethrough) deco.push("line-through");

  if (deco.length > 0) el.style.textDecoration = deco.join(" ");
  if (chp.colorindex != 0) {
    var color = this._doc._lookupColor(chp.colorindex);
    if (color != null) el.style.color = RTFJS._colorToStr(color);
  }
  el.style.fontSize = Math.floor(chp.fontsize / 2) + "pt";
};
...
RTFJS.RenderContainer.prototype.finalize = function () {
  ...
  if (this._sub == null) throw new RTFJS.Error("Container already finalized");

  var cont = this.getContent();
  var len = this._sub.length;
  for (var i = 0; i < len; i++) {
    var element = this._finalizeSub(this._sub[i], this._pap);
    if (element != null) cont.append(element);
  }
  delete this._sub;
  return this._element;
};
...
RTFJS.RenderParagraphContainer.prototype = Object.create(
  RTFJS.RenderContainer.prototype
);
...
RTFJS.RenderParagraphContainer.prototype.applyPap = function (
  el,
  pap,
  chp,
  ismaindiv
  ...
  if (ismaindiv) {
    if (pap.spacebefore != 0)
      el.style.marginTop = RTFJS._twipsToPt(pap.spacebefore) + "pt";
    else el.style.marginTop = "";
    if (pap.spaceafter != 0)
      el.style.marginBottom = RTFJS._twipsToPt(pap.spaceafter) + "pt";
    else el.style.marginBottom = "";
    if (chp != null) el.style.minHeight = Math.floor(chp.fontsize / 2) + "pt";
  } else {
...
RTFJS.RenderParagraphContainer.prototype._finalizeSub = function (
  sub,
  parentPap
) {
  var element = sub.container.finalize();
  if (element) this.applyPap(element, sub.pap ? sub.pap : parentPap, sub.chp);
  return element;
};

RTFJS.RenderParagraphContainer.prototype.finalize = function () {
  RTFJS.log("[rtf] finalizing paragraph");
  if (this._sub == null) throw new RTFJS.Error("Paragraph already finalized");
  if (this._sub.length > 0)
...
RTFJS.RenderTableContainer = function (doc) {
  ...
  elem.classList.add("mat-table");
  elem.classList.add("cdk-table");

  RTFJS.RenderContainer.call(this, doc, "table", elem, null);
  this._rows = [];
  this._row = null;
  this._cell = null;
};

RTFJS.RenderTableContainer.prototype = Object.create(
  RTFJS.RenderContainer.prototype
);

RTFJS.RenderTableContainer.prototype.appendCell = function () {
  RTFJS.log("[rtf] Table appending cell");
  if (this._row == null) this.appendRow();
  var tdele = document.createElement("td");
  tdele.classList.add("mat-cell");
  tdele.classList.add("cdk-cell");

  this._cell = {
    element: this._row.element.appendChild(tdele),
    sub: [],
  };
...
RTFJS.RenderTableContainer.prototype.appendRow = function () {
  ...
  trele.classList.add("mat-row");
  trele.classList.add("cdk-row");

  this._row = {
    element: this._element.appendChild(trele),
    cells: [],
  };
...
RTFJS.RenderTableContainer.prototype.finishCell = function () {
  ...
  if (len > 0) {
    if (this._row == null) this.appendRow();
    if (this._cell == null) this.appendCell();

    for (var i = 0; i < len; i++) this._cell.sub.push(this._sub[i]);
    this._sub = [];
  }
...
RTFJS.RenderTableContainer.prototype.finalize = function () {
  ...
  for (var r = 0; r < rlen; r++) {
    ...
    for (var c = 0; c < clen; c++) {
      ...
      RTFJS.log(
        "[rtf] Table finalize: row[" + r + "].cell[" + c + "].#subs: " + slen
      );
      for (var s = 0; s < slen; s++) {
        var sub = cell.sub[s];
        var element = sub.container.finalize();
        if (element != null) cell.element.append(element);
      }
    }
  }
...
RTFJS.Renderer.prototype.currentContainer = function (type) {
  ...
  if (type != null) {
    for (var i = len - 1; i >= 0; i--) {
      ...
      if (cont.getType() == type) return cont;
    }
    return null;
  }
...
RTFJS.Renderer.prototype.popContainer = function (type) {
  ...
  if (type != null) {
    ...
    if (!popped)
      throw new RTFJS.Error(
        "No container of type " + type + " on rendering stack"
      );
  } else {
    cont = this._curcont.pop();
    if (cont == null) throw new RTFJS.Error("No container on rendering stack");
  }
  return cont;
};
...
RTFJS.Renderer.prototype._appendToPar = function (content, newsubpar) {
  if (newsubpar == true) {
    ...
    if (content != null) this._curpar.push(content);
  } else if (content != null) {
    if (
      this._curcont.length > 0 &&
      this._curcont[this._curcont.length - 1] instanceof
        RTFJS.RenderTableContainer &&
...
RTFJS.Renderer.prototype.finishRow = function () {
  ...
  if (table == null) throw new RTFJS.Error("No table on rendering stack");
  table.finishRow();
};
...
RTFJS.Renderer.prototype.buildRenderedPicture = function (element) {
  if (element == null)
    element = document.createElement("span").textContent =
...
RTFJS.Renderer.prototype.buildPicture = function (mime, data) {
  ...
  if (data != null) {
    element = document
      .createElement("img")
      .setAttribute("src", "data:" + mime + ";base64," + btoa(data));
  } else {
    var err = "image type not supported";
    if (typeof mime === "string" && mime != "") err = mime;
    element = document.createElement("span");
    element.textContent = "[" + mime + "]";
  }
  return new RTFJS.RenderElement(this._doc, "picture", element);
};
...
RTFJS.Renderer.prototype.buildDom = function () {
  if (this._dom != null) return this._dom;
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (element) this._dom.push(element);
  }
...
RTFJS.Document.prototype._lookupColor = function (idx) {
  if (idx == 0) {
    if (this._autoColor == null) return null;
...
RTFJS.Document.prototype.parse = function (blob, renderer) {
  ...
  var parser = {
    ...
    readBlob: function (cnt) {
      ...
      var view = new Uint8Array(buf);
      for (var i = 0; i < cnt; i++) view[i] = this.data[this.pos + i];
      return buf;
    },
  };
  ...
  var findParentDestination = function (dest) {
    ...
    while (state != null) {
      if (state.destination == null) break;
      if (state.destination._name == dest) return state.destination;
      state = state.parent;
    }
  ...
  DestinationFormattedTextBase.prototype.handleKeyword = function (
    keyword,
    param
  ) {
    this._records.push(function (rtf) {
  ...
  DestinationFormattedTextBase.prototype.apply = function () {
    ...
    if (rtf == null)
      throw new RTFJS.Error(
        "Destination " + this._name + " is not child of rtf destination"
      );

    var len = this._records.length;
    var doRender = true;
    if (this.renderBegin != null) doRender = this.renderBegin(rtf, len);

    if (doRender) {
      for (var i = 0; i < len; i++) {
        this._records[i](rtf);
      }
      if (this.renderEnd != null) this.renderEnd(rtf, len);
    }
  ...
  var rtfDestination = function () {
    var cls = function (name, param) {
      ...
      if (param && param != 1) throw new RTFJS.Error("Unsupported rtf version");
      parser.version = 1;
    ...
    cls.prototype._updateFormatIns = function (ptype, props) {
      ...
      if (changed == null) this._propchanged[ptype] = changed = props;
      if (changed != props) {
    ...
    var _genericFormatSetNoParam = function (ptype, prop, val) {
      return function (param) {
        ...
        props[prop] = val;
        RTFJS.log(
          "[rtf] state." + ptype + "." + prop + " = " + props[prop].toString()
        );
    ...
    var _genericFormatSetMemberVal = function (
      ptype,
      prop,
      member,
      defaultValOrFunc
      ...
      return function (param) {
        ...
        else val = param == null ? defaultValOrFunc : param;
        members[member] = val;
        RTFJS.log(
          "[rtf] state." +
            ptype +
        ...
        if (member != null) {
          ...
          RTFJS.log(
            "[rtf] state.table." +
              member +
              " = " +
              parser.state.table[member].toString()
          );
        } else {
          if (parser.state.table != null) {
    ...
    var _charFormatHandlers = {
      ...
      loch: _genericFormatSetNoParam(
        "pap",
        "charactertype",
        RTFJS.CHARACTER_TYPE.LOWANSI
      ),
      hich: _genericFormatSetNoParam(
        "pap",
        "charactertype",
        RTFJS.CHARACTER_TYPE.HIGHANSI
      ),
      dbch: _genericFormatSetNoParam(
        "pap",
        "charactertype",
        RTFJS.CHARACTER_TYPE.DOUBLE
      ),
      ...
      ul: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.CONTINUOUS,
        RTFJS.UNDERLINE.NONE
      ),
      uld: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.DOTTED,
        RTFJS.UNDERLINE.NONE
      ),
      uldash: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.DASHED,
        RTFJS.UNDERLINE.NONE
      ),
      uldashd: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.DASHDOTTED,
        RTFJS.UNDERLINE.NONE
      ),
      uldashdd: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.DASHDOTDOTTED,
        RTFJS.UNDERLINE.NONE
      ),
      uldb: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.DOUBLE,
        RTFJS.UNDERLINE.NONE
      ),
      ulhwave: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.HEAVYWAVE,
        RTFJS.UNDERLINE.NONE
      ),
      ulldash: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.LONGDASHED,
        RTFJS.UNDERLINE.NONE
      ),
      ulnone: _genericFormatSetNoParam(
        "chp",
        "underline",
        RTFJS.UNDERLINE.NONE
      ),
      ulth: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.THICK,
        RTFJS.UNDERLINE.NONE
      ),
      ulthd: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.THICKDOTTED,
        RTFJS.UNDERLINE.NONE
      ),
      ulthdash: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.THICKDASHED,
        RTFJS.UNDERLINE.NONE
      ),
      ulthdashd: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.THICKDASHDOTTED,
        RTFJS.UNDERLINE.NONE
      ),
      ulthdashdd: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.THICKDASHDOTDOTTED,
        RTFJS.UNDERLINE.NONE
      ),
      ululdbwave: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.DOUBLEWAVE,
        RTFJS.UNDERLINE.NONE
      ),
      ulw: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.WORD,
        RTFJS.UNDERLINE.NONE
      ),
      ulwave: _genericFormatOnOff(
        "chp",
        "underline",
        RTFJS.UNDERLINE.WAVE,
        RTFJS.UNDERLINE.NONE
      ),
      ...
      sbknone: _genericFormatSetNoParam(
        "sep",
        "breaktype",
        RTFJS.BREAKTYPE.NONE
      ),
      sbkcol: _genericFormatSetNoParam("sep", "breaktype", RTFJS.BREAKTYPE.COL),
      sbkeven: _genericFormatSetNoParam(
        "sep",
        "breaktype",
        RTFJS.BREAKTYPE.EVEN
      ),
      sbkodd: _genericFormatSetNoParam("sep", "breaktype", RTFJS.BREAKTYPE.ODD),
      sbkpage: _genericFormatSetNoParam(
        "sep",
        "breaktype",
        RTFJS.BREAKTYPE.PAGE
      ),
      pgnx: _genericFormatSetMemberVal("sep", "pagenumber", "x", 0),
      pgny: _genericFormatSetMemberVal("sep", "pagenumber", "y", 0),
      pgndec: _genericFormatSetNoParam(
        "sep",
        "pagenumberformat",
        RTFJS.PAGENUMBER.DECIMAL
      ),
      pgnucrm: _genericFormatSetNoParam(
        "sep",
        "pagenumberformat",
        RTFJS.PAGENUMBER.UROM
      ),
      pgnlcrm: _genericFormatSetNoParam(
        "sep",
        "pagenumberformat",
        RTFJS.PAGENUMBER.LROM
      ),
      pgnucltr: _genericFormatSetNoParam(
        "sep",
        "pagenumberformat",
        RTFJS.PAGENUMBER.ULTR
      ),
      pgnlcltr: _genericFormatSetNoParam(
        "sep",
        "pagenumberformat",
        RTFJS.PAGENUMBER.LLTR
      ),
      qc: _genericFormatSetNoParam(
        "pap",
        "justification",
        RTFJS.JUSTIFICATION.CENTER
      ),
      ql: _genericFormatSetNoParam(
        "pap",
        "justification",
        RTFJS.JUSTIFICATION.LEFT
      ),
      qr: _genericFormatSetNoParam(
        "pap",
        "justification",
        RTFJS.JUSTIFICATION.RIGHT
      ),
      qj: _genericFormatSetNoParam(
        "pap",
        "justification",
        RTFJS.JUSTIFICATION.JUSTIFY
      ),
    ...
    cls.prototype.apply = function () {
      RTFJS.log("[rtf] apply()");
      this.flushProps();
      for (var prop in this._metadata) inst._meta[prop] = this._metadata[prop];
  ...
  var genericPropertyDestination = function (parentdest, metaprop) {
    ...
    cls.prototype.apply = function () {
      ...
      if (dest.setMetadata == null)
        throw new RTFJS.Error(
          "Destination " + parentdest + " does not accept meta data"
        );
  ...
  var metaPropertyDestination = function (metaprop) {
    ...
    cls.prototype.apply = function () {
      ...
      if (info == null)
        throw new RTFJS.Error(
          "Destination " + this._name + " must be within info destination"
        );
  ...
  var fonttblDestinationSub = function () {
    ...
    cls.prototype.appendText = function (text) {
      if (this.fontname == null) this.fontname = text;
      else this.fontname += text;
    };
    cls.prototype.apply = function () {
      if (this.index == null) throw new RTFJS.Error("No font index provided");
      if (this.fontname == null) throw new RTFJS.Error("No font name provided");
      ...
      for (var idx in this._fonts) {
        RTFJS.log(
          "[fonttbl][" +
            idx +
            "] index = " +
            this._fonts[idx].fontname +
            " alternative: " +
            this._fonts[idx].altfontname
        );
      }
  ...
  var genericSubTextPropertyDestination = function (
    name,
    parentDest,
    propOrFunc
  ) {
    ...
    cls.prototype.apply = function () {
      ...
      if (dest[propOrFunc] == null)
        throw new RTFJS.Error(
          this._name +
            " destination cannot find member + " +
            propOrFunc +
            " in " +
            parentDest +
            " destination"
        );
      if (dest[propOrFunc] instanceof Function) dest[propOrFunc](this.text);
      else dest[propOrFunc] = this.text;
    };
    return cls;
  };
  ...
  var colortblDestination = function () {
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (this._current == null) {
          if (this._autoIndex != null)
            throw new RTFJS.Error(
              "colortbl cannot define more than one auto color"
            );
          ...
          if (this._current.tint < 255 && this._current.shade < 255)
            throw new RTFJS.Error(
              "colortbl cannot define shade and tint at the same time"
            );
        }
    ...
    var _validateColorValueRange = function (keyword, param) {
      if (param == null) throw new RTFJS.Error(keyword + " has no param");
      if (param < 0 || param > 255)
        throw new RTFJS.Error(keyword + " has invalid param value");
      return param;
    };
    cls.prototype.handleKeyword = function (keyword, param) {
      if (this._current == null) this._startNewColor();
    ...
    cls.prototype.apply = function () {
      ...
      if (this._autoIndex == null) this._autoIndex = 0;
  ...
  var stylesheetDestination = function () {
    ...
    cls.prototype.apply = function () {
      ...
      for (var idx in this._stylesheets)
        RTFJS.log(
          "[stylesheet] [" + idx + "] name: " + this._stylesheets[idx].name
        );
  ...
  var fieldDestination = function () {
    ...
    cls.prototype.setInst = function (inst) {
      ...
      if (this._parsedInst != null)
        throw new RTFJS.Error(
          "Field cannot have multiple fldinst destinations"
        );
    ...
    cls.prototype.setResult = function (inst) {
      if (this._result != null)
        throw new RTFJS.Error(
          "Field cannot have multiple fldrslt destinations"
        );
      this._result = inst;
    };
    return cls;
  ...
  var fldinstDestination = function () {
    ...
    cls.prototype.apply = function () {
      ...
      if (field == null)
        throw new RTFJS.Error(
          "fldinst destination must be child of field destination"
        );
    ...
    cls.prototype.parseType = function () {
      ...
      if (sep > 0) {
        ...
        if (data.length >= 2 && data[0] == '"') {
          ...
          if (end >= 1) data = data.substring(1, end);
        }
  ...
  var fldrsltDestination = function () {
    ...
    cls.prototype.renderBegin = function (rtf, records) {
      ...
      if (field != null) {
        ...
        if (inst != null) return inst.renderFieldBegin(field, rtf, records);
      }
      return false;
    };
    cls.prototype.renderEnd = function (rtf, records) {
      ...
      if (field != null) {
        var inst = field.getInst();
        if (inst != null) inst.renderFieldEnd(field, rtf, records);
      }
    };
    return cls;
  };
  ...
  var pictDestination = function () {
            ...
            return {
              load: function () {
                try {
                  ...
                  if (e instanceof EMFJS.Error) return e.message;
                  else throw e;
                }
              },
              render: function (img) {
            ...
            return {
              load: function () {
                try {
                  ...
                  if (e instanceof WMFJS.Error) return e.message;
                  else throw e;
                }
              },
              render: function (img) {
    ...
    cls.prototype.apply = function () {
      ...
      if (typeof type === "function") {
          ...
          if (inst._settings.onPicture != null) {
            inst._renderer.addIns(
              (function (isLegacy) {
                return function () {
                  var renderer = this;
                  var elem = inst._settings.onPicture.call(
                    inst,
                    isLegacy,
                    function () {
                  ...
                  if (elem != null) this.appendElement(elem);
                };
              })(isLegacy)
            );
          } else {
        ...
        var doRender = function (rendering) {
          var bin =
            blob != null ? RTFJS._blobToBinary(blob) : RTFJS._hexToBinary(text);
          if (type !== "") {
            if (rendering) {
  ...
  var destinations = {
    ...
    falt: genericSubTextPropertyDestination(
      "falt",
      "fonttbl:sub",
      "setAltFontName"
    ),
  ...
  var applyDestination = function (always) {
    ...
    if (dest != null) {
      if (
        always ||
        parser.state.parent == null ||
        parser.state.parent.destination != parser.state.destination
      ) {
        if (dest.apply != null) dest.apply();
  ...
  var applyText = function () {
    if (parser.text.length > 0) {
      ...
      if (
        dest != null &&
        dest.appendText != null &&
        !parser.state.skipdestination
      )
  ...
  var pushState = function (forceSkip) {
    ...
    if (forceSkip) parser.state.skipdestination = true;

    var dest = parser.state.destination;
    if (dest != null && !parser.state.skipdestination) {
      if (dest.sub != null) {
        var sub = dest.sub();
        if (sub != null) parser.state.destination = sub;
      }
    }
  };
  ...
  var popState = function () {
    var state = parser.state;
    if (state == null) throw new RTFJS.Error("Unexpected end of state");
  ...
  processKeyword = function (keyword, param) {
    ...
    switch (keyword) {
      ...
      case "uc":
        if (param != null && param >= 0) parser.state.ucn = param;
        break;
      case "u":
        if (param != null) {
          ...
          appendText(
            symbol !== undefined ? symbol : String.fromCharCode(param)
          );
      ...
      case "bin":
        ...
        if (param < 0) throw new RTFJS.Error("Binary data with invalid length");
  ...
  parseKeyword = function (process) {
    if (parser.state == null) throw new RTFJS.Error("No state");
      ...
      if (ch == "'") {
        ...
        if (process != null) {
          ...
          if (parser.state.chp.hasOwnProperty("fontfamily")) {
            ...
            if (
              inst._fonts != undefined &&
              inst._fonts[idx] != null &&
              inst._fonts[idx].charset != undefined
            )
          ...
          switch (ch) {
            ...
            case "}":
              if (initialState == parser.state) {
                parser.unreadChar();
                break main_loop;
              } else if (popState() == initialState) break main_loop;
              break;
            case "\\":
              parseKeyword(!skip ? process : null);
              break;
            default:
              if (!skip) appendText(ch);
              break;
          }
        }
      }
  ...
  if (parser.version == null) throw new RTFJS.Error("Not a valid rtf document");
  if (parser.state != null) throw new RTFJS.Error("File truncated");
};

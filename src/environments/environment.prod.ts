export const environment = {
  production: true,
  clientId: "hispwa",
  ApiUrl: "",
  cookieUrl: "",
  //dev environment urls
  CommonApiUrl: "http://172.30.0.16:1009/",
  PatientApiUrl: "http://172.30.0.16:1008/",
  IdentityServerUrl: "https://MaxHIS-Idsrv-dev.maxhealthcare.in/",
  IentityServerRedirectUrl: "http://172.30.0.16/",
  ReportsApiUrl: "https://MaxHIS-Reports-dev.maxhealthcare.in/",
  BillingApiUrl: "http://172.30.0.16:1007/",

  //local envionment urls for testing
  // ReportsApiUrl:'https://MaxHIS-Reports-dev.maxhealthcare.in/',
  // BillingApiUrl:'http://172.30.0.16:1007/',
  // IdentityServerUrl: 'https://localhost/',
  // IentityServerRedirectUrl:'http://localhost:8100/',
  //   CommonApiUrl : 'http://172.30.0.16:1009/',
  // PatientApiUrl : 'http://172.30.0.16:1008/',

  // CommonApiUrl:'https://MaxHIS-Common-sit.maxhealthcare.in/',
  //   PatientApiUrl:'https://MaxHIS-OpReg-sit.maxhealthcare.in/',
  //   ReportsApiUrl:'https://MaxHIS-Reports-sit.maxhealthcare.in/',
  //   IdentityServerUrl:'https://MaxHIS-Idsrv-sit.maxhealthcare.in/',
  //   IentityServerRedirectUrl:'https://MaxHIS-sit.maxhealthcare.in/',

  //UAT
  // ReportsApiUrl: "https://MaxHIS-Reports-UAT.maxhealthcare.in/",
  // IdentityServerUrl: "https://MaxHIS-Idsrv-UAT.maxhealthcare.in/",
  // IentityServerRedirectUrl: "https://MaxHIS-uat.maxhealthcare.in/",
  // PatientApiUrl: "https://MaxHIS-Common-uat.maxhealthcare.in/",
  // CommonApiUrl: "https://MaxHIS-OpReg-uat.maxhealthcare.in/",
};
